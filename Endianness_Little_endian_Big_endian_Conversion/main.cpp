...
int main() {
    ...
    std::cout << "reading a number encoded as big endian in a big-endian-CPU-way:    from MSB [0th byte] to LSB [3rd byte] = DECODED AS SAME NUMBER WITH BIG ENDIAN CPU - different with little endian CPU\n";
    ...
    std::cout << "reading a number encoded as little endian without conversion in a little-endian-CPU-way: from MSB [3rd byte] to LSB [0th byte] = DECODED AS DIFFERENT NUMBER WITH BIG ENDIAN CPU\n";
    uint8_t big_endian_number_decoded_as_little_endian[4];
    uint8_t big_endian_number_converted_to_little_endian_2[4];
    int8_t destinationBytenumber {0};

    NumberUnion big_endian_number_decoded_as_little_endian_union[4];
    ...
    for (int8_t sourceByteNumber = 3; sourceByteNumber >= 0; --sourceByteNumber) {
        ...
        big_endian_number_converted_to_little_endian_2[destinationBytenumber] = number_array_big_endian[sourceByteNumber];
    ...
    // TODO avoiding reinterpret cast with union
