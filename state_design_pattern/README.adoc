...
// TODO further development - tasks postponed in order to keep accuracy with the original state machine diagram on refactoring.guru site
        ...
        // TODO v2: in 'Transition' class change references to pointers to avoid duplicate code for setting transition type from enum in concrete states
        // TODO v2.1: make 'State' class a Template, that will call a private transition-instance-setter first ('transition' will be a member of the 'Document' class) and then will have a pure virtual hook function overriden in derived classes
