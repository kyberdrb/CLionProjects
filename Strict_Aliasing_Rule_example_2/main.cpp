...
int main() {
    ...
    uint32_t* buff = (uint32_t*) malloc(sizeof(Msg));   // allocate a buffer of size of 32 bits (4 bytes)
    Msg* aliasedMsg = (Msg*) buff;                      // alias the buffer with the Msg struct
                                                        // Dangerous! Implicit assumption that 'int' and 'uint32_t' have the same size - 4 bytes
    ...
    printf("\n---------------------------------------------------");
    printf("\nWe're lucky. No undefined behavior occurs\nbecause 'int' and 'uint32_t' have the same size on this platform.\n");
    printf("---------------------------------------------------\n\n");
