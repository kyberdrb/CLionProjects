...
public:
    ...
    TwinResource(ResourceSpecifier1* rs1, double aValue)
            : firstResource(dynamic_cast<ResourceHandle1*>(allocResource(rs1)))
            , secondResource(dynamic_cast<ResourceHandle2*>(allocResource(determineResourceSpec2(aValue)))) {
        verifyResourcesAreAllocated ();
    }

    TwinResource(std::string aName, ResourceSpecifier* rs2)
            : firstResource(dynamic_cast<ResourceHandle1*>(allocResource(determineResourceSpec1(aName))))
            , secondResource(dynamic_cast<ResourceHandle2*>(allocResource(rs2))) {
        verifyResourcesAreAllocated ();
    }
...
private:
    ...
    ResourceSpecifier* determineResourceSpec1(std::string basicString) {
        return new ResourceSpecifier1();
    }
